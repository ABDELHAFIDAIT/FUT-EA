        ...
        data.forEach(p => {
            if (p.id === playerNum) {
                ...
                dragItem.addEventListener("dragend", function (event) {
                    ...
                    newf.setAttribute('id', `player-${p.id}`);
                    let totalRat = (p.pace + p.shooting + p.passing + p.dribbling + p.defending + p.physical)/6 ;
                    newf.innerHTML = `
                                    <div class='theCard'>
                                        <div class='topCard'>
                                            <span class='topCardLeft'>
                                            <p id="Rating">${totalRat}</p>
...
function makethecards() {
    ...
    for(let j = 0;j<data.length;j++){
            ...
            if (i === 0 && p.position === 'gk') {
                ...
                document.getElementById(`player-${p.id}`).style.display = 'none';
                newf.setAttribute('id', `player-${p.id}`);
                let totalRating = (p.pace + p.shooting + p.passing + p.dribbling + p.defending + p.physical)/6 ;
                newf.innerHTML = `
                    <div class='theCard'>
                        <div class='topCard'>
                            <span class='topCardLeft'>
                                <p id="Rating">${totalRating}</p>
                                <p id='Pos'>${p.position}</p>
                                <img src='${p.flag}'>
                            </span>
                            <span class='topCardRight'>
                ...
                document.getElementById(`player-${p.id}`).style.display = 'none';
                newf.setAttribute('id', `player-${p.id}`);
                let totRat = (p.pace + p.shooting + p.passing + p.dribbling + p.defending + p.physical)/6 ;
                newf.innerHTML = `
                    <div class='theCard'>
                        <div class='topCard'>
                            <span class='topCardLeft'>
                                <p id="Rating">${totRat}</p>
                                <p id='Pos'>${p.position}</p>
                                <img src='${p.flag}'>
                            </span>
                            <span class='topCardRight'>
